        ...
        post('create appointement') do
            response(200, 'successful') do
              ...
              response '201', 'appointement created' do
                ...
                let(:user) {  User.create!(name: 'Ayesha', email: 'ayesha@gmail.com', password: 'topsecret', password_confirmation: 'topsecret') }
                let(:appointement) { { user: user, doctor: doctor} }
                run_test!
              end
        
              response '422', 'invalid request' do
                let(:user) {  User.create!(name: 'Ayesha', email: 'ayesha@gmail.com', password: 'topsecret', password_confirmation: 'topsecret') }
                let(:appointement) { { user: user } }
                run_test!
              end
            end
